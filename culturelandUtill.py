...
class KeyPad():
    def __init__(self, crypto, key_type, skip_data, keys):
        ...
        self.keys = keys
        self.lower = self._calc_skipped_chars(lower)
        self.upper = self._calc_skipped_chars(upper)
        self.special = self._calc_skipped_chars(special)
...
class Engine:
    ...
    pw: str
        ...
        """
        CultureLand Information

        [WARNING]
        해당 파이썬 스크립트는 (주)컬처랜드 와 무관한 '3rd Party Unofficial' 입니다.
        해당 프로그램을 사용하며 생기는 모든 문제의 책임은 사용자 본인에게 있습니다.

        [INFORMATION]
        일반 충전과 동일하게, 1일 10회 오류제한이 존재합니다.
        배포되는 코드에는 오류제한 방지코드가 포함되어있지 않습니다.
        배포되는 코드에는 선물하기 기능또한 포함되지 않으며, 오직 충전/잔액확인 만 가능합니다.
        ...
        client.getBalance() #컬처랜드 잔액 확인 (안심 금고 포함)

        client.charge(pin) #핀번호 충전
        """
        self.session = session()
        self.config = {'IDENTIFICATION': id, 'PASSWORD': pw, 'BALANCE_RETURN_FORM': ['$[보유금액]', '$[안심금고금액]', '$[한도액]']}
    
    def login(self):
        username = self.config['IDENTIFICATION']
        ...
        if len(username) > 12 or len(username) < 6 or username is None or username == "" or len(password) > 12 or len(password) < 6 or password is None or password == "":
            ...
            self.mtk = mTransKey(self.session)
            ...
            res: Response = self.session.post("https://m.cultureland.co.kr/mmb/loginProcess.do", data=data, cookies={'KeepLoginConfig': 'lkjasf'})
            if not res.status_code == 200:
                return [False, '서버에서 정상적인 상태코드를 반환히지 않았습니다.']
            else:
                if '입력하신 아이디 또는 비밀번호가 틀립니다' in res.text:
                    return [False, '아이디 또는 비밀번호가 일치하지 않습니다.']
                else:
                    res: Response = self.session.post("https://m.cultureland.co.kr/mmb/isLogin.json")
                    if res.text == 'false':
                        return [False, '로그인에 실패하였습니다.']
                    elif res.text == 'true':
                        return [True, '성공']
    ...
    def getBalance(self):
        res: Response = self.session.post("https://m.cultureland.co.kr/gft/chkGiftLimitAmt.json")
        if not res.status_code == 200:
            return [False, '서버에서 정상적인 상태코드를 반환히지 않았습니다.']
        ...
        try:
            if chkGiftLimitAmt['errMsg'] == '정상':
                ...
                if self.config['BALANCE_RETURN_FORM'] != None:
                    lists = []
                    lists.append(True)
                    for i in self.config['BALANCE_RETURN_FORM']:
                        if i == '$[보유금액]':
                            lists.append(int(giftVO['balanceAmt']))
                        elif i == '$[안심금고금액]':
                            lists.append(int(giftVO['safeAmt']))
                        elif i == '$[한도액]':
                            lists.append(int(giftVO['ccashRemainAmt']))
                        else:
                            pass
                    if len(lists) == 1:
                        return [True, int(giftVO['balanceAmt'])]
                    else:
                        return lists
                else:
                    return [True, int(giftVO['balanceAmt'])]
            else:
                return [False, 'Status 오류.']
        except:
            if chkGiftLimitAmt['resultMessage'] == "로그인이 필요한 서비스 입니다.":
                return [False, '로그인이 필요한 서비스 입니다.']
        return [False, '알 수 없는 오류.']
    
    def charge(self, code: str):
        if code is None:
            return [False, '상품권 번호를 입력하여주세요.']
        res: Response = self.session.get("https://m.cultureland.co.kr/csh/cshGiftCard.do")
        if not res.status_code == 200:
            return [False, '서버에서 정상적인 상태코드를 반환히지 않았습니다.']
        if len(code) != 19 and len(code) != 21:
            return [False, '상품권 길이가 일치자히 않습니다.']
        codes = code.split("-")
        ...
        res: Response = self.session.post("https://m.cultureland.co.kr/csh/cshGiftCardProcess.do", data=data)
